...
function colorToRgb(color) {
  ...
  const [r, g, b] = match[1].split(",").map((channel) => parseRgbChannel(channel.trim()));
...
function getReadableSurfaceColors(surfaceColor) {
  const lightSurface = isLightColor(surfaceColor);
  return {
    text: lightSurface ? "#111827" : "#f8fafc",
    muted: lightSurface ? "rgba(17, 24, 39, 0.66)" : "rgba(248, 250, 252, 0.72)",
  };
}
...
export function normalizeThemeState(input = {}) {
  ...
  return {
    ...
    sidebarSettings: {
      ...DEFAULT_THEME_STATE.sidebarSettings,
      ...(input.sidebarSettings || {}),
    },
    dashboardSettings: {
      ...DEFAULT_THEME_STATE.dashboardSettings,
...
export function getThemeStateFromUserData(userData = {}) {
  return normalizeThemeState({
    ...
    themeMode: userData.themeMode || userData.settings?.theme || DEFAULT_THEME_STATE.themeMode,
    customColors: userData.customColors || userData.theme?.customColors,
    sidebarSettings: userData.sidebarSettings || userData.theme?.sidebarSettings,
    dashboardSettings: userData.dashboardSettings || userData.theme?.dashboardSettings,
    purchasedThemes: userData.purchasedThemes || userData.ownedThemes || [],
    favoriteThemes: userData.favoriteThemes || userData.themeFavorites || [],
    recentThemes: userData.recentThemes || userData.themeRecentThemes || [],
    customThemes: userData.customThemes || userData.themeCustomThemes || [],
  });
}
...
export function buildThemeVariables(themePreset, themeState = {}) {
  ...
  const custom = themeState.customColors || {};
  const palette = {
    ...preset.colors,
    ...custom,
  };

  const glassBlur = preset.glassmorphism?.blur || 16;
  const surfaceBase = palette.surface || palette.card || palette.surfaceElevated || palette.background;
  const surfaceSolid = palette.surfaceElevated || palette.card || palette.background;
  const surfaceColors = getReadableSurfaceColors(surfaceSolid);
  const shadow = palette.background === "#050816" || palette.background === "#04110a"
  ...
  return {
    ...
    "--theme-surface-solid": surfaceSolid,
    "--theme-surface-text": surfaceColors.text,
    "--theme-surface-muted": surfaceColors.muted,
    "--theme-sidebar-bg": palette.sidebar,
    "--theme-sidebar-text": palette.sidebarText || "#f8fafc",
    ...
    "--theme-glass-bg": `rgba(255, 255, 255, ${preset.glassmorphism?.opacity ?? 0.84})`,
    "--theme-glass-border": `rgba(255,255,255,${preset.glassmorphism?.enabled ? 0.22 : 0.12})`,
    "--theme-glass-blur": `${glassBlur}px`,
    "--theme-shadow": shadow,
    "--theme-chart-1": palette.chart?.[0] || palette.accent,
    "--theme-chart-2": palette.chart?.[1] || palette.accent2,
    "--theme-chart-3": palette.chart?.[2] || palette.accent,
    "--theme-chart-4": palette.chart?.[3] || palette.accent2,
  };
}
...
export function getRandomThemePreset(ownedOnly = false, userData = {}) {
  const pool = ownedOnly
    ? THEME_PRESETS.filter((preset) => preset.isFree || isThemeOwned(preset.id, userData))
    : THEME_PRESETS;
